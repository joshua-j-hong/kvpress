...
for i in "${!press_names[@]}"; do
  ...
  (
    for compression_ratio in "${compression_ratios[@]}"; do
      ...
      echo "python evaluate.py --dataset $dataset --data_dir $data_dir --model $model --press_name $press --compression_ratio $compression_ratio --device cuda:$i"
